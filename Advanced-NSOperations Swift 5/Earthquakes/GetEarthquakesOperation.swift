...
class GetEarthquakesOperation: GroupOperation {
    ...
    */
    init(context: NSManagedObjectContext, completionHandler: @escaping () -> Void) {
        ...
        */
        downloadOperation = DownloadEarthquakesOperation(cacheFile: cacheFile as NSURL)
        parseOperation = ParseEarthquakesOperation(cacheFile: cacheFile as NSURL, context: context)
        
        let finishOperation = BlockOperation.init(mainQueueBlock: completionHandler)
    ...
    override func operationDidFinish(operation: Operation, withErrors errors: [NSError]) {
        if let firstError = errors.first,
            (operation === downloadOperation || operation === parseOperation) {
